...
def load_system_prompts():
    try:
        ...
        logger.error("[load_system_prompts]yaml配置中无main_prompt_path配置项")
        raise e
...
def load_report_prompts():
    try:
        ...
        logger.error("[load_report_prompts]yaml配置中无report_prompt_path配置项")
        raise e
...
def load_rag_prompts():
    try:
        rag_prompt_path = get_abs_path(
            prompts_config["rag_summarize_prompt_path"])
    except KeyError as e:
        logger.error("[load_rag_prompts]yaml配置中无rag_summarize_prompt_path配置项")
        raise e
